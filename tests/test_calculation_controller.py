...
from scientific_calculator.calculation_worker import (
    CalculationPayload,
    CalculationRequest,
...
class StubbornCleanupProcess(CleanupProcess):
    def terminate(self):
        self.calls.append("terminate")
...
def test_worker_allows_only_explicit_operations():
    engine = ScientificCalculatorEngine()
    with pytest.raises(ValueError):
        build_calculation_request(engine, "__class__", (), {})

    request = CalculationRequest(
        operation="__class__",  # type: ignore[arg-type]  # Deliberately malformed IPC input.
        args=(),
        kwargs={},
...
def test_cleanup_kills_a_stubborn_process_once_and_stops_rescheduling():
    scheduler = ManualScheduler()
    controller = CalculationController(scheduler)
    process = StubbornCleanupProcess()

    controller._poll_cleanup(process, terminate=True, attempts=0)
