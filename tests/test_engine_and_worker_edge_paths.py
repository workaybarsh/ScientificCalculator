"""Engine and worker-controller error handling and cleanup paths.
...
def test_parser_security_helpers_classify_untrusted_ast_and_resource_inputs(engine):
    assert math.isinf(parser_module.constant_numeric_approx(ast.parse("2**100000", mode="eval").body))
    assert parser_module.constant_numeric_approx(ast.parse("(-1)**0.5", mode="eval").body) is None
    assert parser_module.constant_numeric_approx(ast.parse("factorial(171)", mode="eval").body) == math.inf
    assert parser_module.constant_numeric_approx(ast.parse("nCr(3,4)", mode="eval").body) is None
    assert parser_module.evaluated_numeric_approx(complex(math.inf, 0)) == math.inf
    assert parser_module.evaluated_numeric_approx(sp.oo) == math.inf
    assert parser_module.evaluated_numeric_approx(sp.Rational(3, 2)) == pytest.approx(1.5)
...
def test_result_formatting_handles_arrays_equalities_and_nonfinite_numeric_values(engine):
    assert engine.format_result(np.array([[1.0, 2.0]])) == "[[1.000, 2.000]]"
    x = sp.Symbol("x")
