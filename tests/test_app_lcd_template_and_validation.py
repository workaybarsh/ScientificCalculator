"""LCD menu routing, template editing, and field validation.
...
@pytest.mark.parametrize(("mode", "action", "order"), [("Integral", "Double Integral", "double"), ("Integral", "Triple Integral", "triple")])
def test_lcd_multi_integrals_open_the_numbered_template(mode: str, action: str, order: str) -> None:
    app = _lcd_app()
    app._lcd_flow = {"mode": mode, "source_expression": "x*y", "values": {"calculus_action": action}}
...
def test_lcd_calculus_selector_rejects_an_unknown_operation() -> None:
    app = _lcd_app()
...
def test_ode_template_requires_every_coefficient_before_dispatching() -> None:
    app = _lcd_app()
    app.template_kind = "ode"
    app.template_fields = {"ode_a": "1", "ode_b": "", "ode_c": "1", "ode_f": "0"}
    with pytest.raises(CalculatorError, match="Enter every ODE coefficient"):
...
def test_template_editing_helpers_keep_calculus_text_and_variables_editable() -> None:
    ...
    assert app.template_fields["var"] == ""
...
def test_integral_shortcuts_and_option_menus_reach_visible_mode_specific_actions() -> None:
    ...
    integral._clear_before_interaction_transition = mock.Mock(return_value="x^2")
    integral.start_derivative_template = mock.Mock()
    App.integral_key(integral)
    integral._clear_before_interaction_transition.assert_called_once_with()
    integral.start_derivative_template.assert_called_once_with("x^2")

    integral.shift = False
    integral.mode = "Matrix"
    integral._open_calculus_flow = mock.Mock()
    App.integral_key(integral)
    integral._open_calculus_flow.assert_not_called()
...
def test_lcd_empty_state_helpers_and_settings_reset_recover_without_a_visible_widget_tree() -> None:
    ...
    reset._fit_ui_scale_to_display = mock.Mock(return_value=75)
    reset._rebuild_scaled_ui = mock.Mock()
    reset._lcd_message = mock.Mock()
    assert App.reset_app_settings(reset) is True
    assert reset.requested_ui_scale == 100
    assert reset.ui_scale == 75
    reset._fit_ui_scale_to_display.assert_called_once_with(100)
...
def test_template_and_ac_paths_handle_backspace_history_and_non_workspace_mode_resets() -> None:
    ...
    invalid_variable = _lcd_app()
    invalid_variable.template_kind = "integral"
    invalid_variable.template_fields = {
        "body": "a^2", "lower": "", "upper": "", "var": "aa",
    }
    invalid_variable.template_cursors = {"body": 3, "lower": 0, "upper": 0, "var": 2}
    with pytest.raises(CalculatorError, match="Enter the differential variable"):
        App.evaluate_template(invalid_variable)
...
def test_lcd_edge_states_keep_invalid_selection_navigation_and_empty_tools_safe() -> None:
    ...
    lines = App._polynomial_result_lines(app, 3, [1, 0, 0, -1])
    assert not any("Vertex" in line for line in lines)
...
def test_template_viewport_handles_wide_glyphs_and_editing_boundary_decisions() -> None:
    ...
    # Two ellipses would exactly fill the budget while showing none of the
    # text.  Overrunning it by one glyph is the better trade: a viewport
    # exists to show the characters around the caret.
    assert shown == "…a…"
    assert offset == 50
...
def test_a_constrained_slot_never_hides_its_only_character_behind_an_ellipsis(cursor: int) -> None:
    """A slot too narrow for one glyph must still show the value.

    Template slots reserve a fixed pixel budget measured against Consolas,
    which exists only on Windows.  When Tk substitutes a wider font the
    budget no longer fits a single character, and the truncation loops can
    meet and leave nothing but an ellipsis -- silently hiding, for example,
    the integration variable of a multiple-integral template.
    """
    shown, _caret = App._template_text_view(
        mock.Mock(), "x", cursor, ("Consolas", 17), max_width=1, empty_placeholder="□"
    )
