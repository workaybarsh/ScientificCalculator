...
def test_history_persistence_filters_invalid_records_and_limits_saved_history() -> None:
    ...
    assert app.history_pos == len(expected) - 1
    store.save_history.assert_called_once_with(expected)


def test_history_load_failure_clears_stale_memory_and_clear_history_rolls_back() -> None:
...
def test_settings_save_verifies_both_config_and_history_before_reporting_success() -> None:
    ...
    store = SimpleNamespace(
        ...
        load=mock.Mock(return_value=App._flatten_settings(expected)),
        load_history=mock.Mock(return_value=history),
    )
    ...
    mismatched_config = _app_with_core(history=history)
    mismatched_config.core.settings = settings
    mismatched_config.ui_scale = 125
    mismatched_config.skin_name = "Blue"
    mismatched_config.err = mock.Mock()
    mismatched_config._log_settings_issue = mock.Mock()
    bad_config = dict(expected)
    bad_config["scale"] = 100
    mismatched_config._settings_store = mock.Mock(
        return_value=SimpleNamespace(
            save_state=mock.Mock(),
            load=mock.Mock(return_value=App._flatten_settings(bad_config)),
            load_history=mock.Mock(return_value=history),
        )
    )

    assert App.save_settings_file(mismatched_config, notify=True) is False
    mismatched_config._log_settings_issue.assert_called_once()
    assert mismatched_config.err.call_args.kwargs == {"clear_input": False}

    mismatched_history = _app_with_core(history=history)
    mismatched_history.core.settings = settings
    mismatched_history.ui_scale = 125
    mismatched_history.skin_name = "Blue"
    mismatched_history.err = mock.Mock()
    mismatched_history._log_settings_issue = mock.Mock()
    mismatched_history._settings_store = mock.Mock(
        return_value=SimpleNamespace(
            save_state=mock.Mock(),
            load=mock.Mock(return_value=App._flatten_settings(expected)),
            load_history=mock.Mock(return_value=[]),
        )
    )

    assert App.save_settings_file(mismatched_history, notify=True) is False
    mismatched_history._log_settings_issue.assert_called_once()
    assert mismatched_history.err.call_args.kwargs == {"clear_input": False}
