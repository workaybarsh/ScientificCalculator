"""Settings and history persistence at the profile-storage boundary.

Corrupt payloads, unavailable profile directories, and failing stores must
leave the running session usable.  These tests pin that degradation and the
error helpers that keep a failure visible on the LCD.
"""
...
def test_load_and_persist_history_preserve_entries_when_the_store_cannot_return_history() -> None:
    ...
    assert app.core.history == [("stale", "1")]
...
def test_lcd_message_safely_handles_a_missing_result_widget_without_losing_label_text() -> None:
    app = object.__new__(App)
    App._lcd_message(app, "this must not crash before the UI exists")

    app.result = _Result()
    App._lcd_message(app, "x" * 40)
    assert app.result.messages == ["x" * 40]
