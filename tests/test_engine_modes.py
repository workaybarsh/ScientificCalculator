...
@pytest.mark.parametrize(
    ("left", "right", "message"),
    [
        ([0, 0], [1, 0], "açısı tanımsız"),
        ([1, 0], [0, 0], "açısı tanımsız"),
        ([float("nan"), 0], [1, 0], "vektör verileri"),
    ],
)
def test_vector_angle_rejects_zero_or_nonfinite_norm(engine, left, right, message):
...
@pytest.mark.parametrize("frequency", [2**53, 2**63, 2**53 - 1])
def test_one_variable_statistics_rejects_unsafe_frequency_counts(engine, frequency):
    with pytest.raises(CalculatorError, match="güvenli"):
        engine.one_var_stats([1, 2], freq=[frequency, 1])


def test_one_variable_statistics_uses_python_cumulative_weights(engine):
    result=engine.one_var_stats([1, 3], freq=[2**52, 2**52 - 1])
    assert result["n"] == 2**53 - 1
    assert result["Med"] == pytest.approx(1)


def test_normal_q_is_signed_area_from_the_mean(engine):
    assert engine.normal_Q(-1) < 0
    assert engine.normal_Q(0) == pytest.approx(0)
    assert engine.normal_Q(1) > 0
