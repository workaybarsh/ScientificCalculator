"""Calculus guards around the SciPy, lambdify, and CAS adapters.
...
def test_public_multivariate_integrals_translate_numeric_backend_failures(engine, monkeypatch):
    monkeypatch.setattr(engine, "_try_exact_nested_integral", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(scipy.integrate, "dblquad", lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("down")))
...
def test_public_triple_integral_translates_numeric_backend_failures(engine, monkeypatch):
    monkeypatch.setattr(engine, "_try_exact_nested_integral", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(scipy.integrate, "tplquad", lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("down")))
...
def test_numeric_helpers_handle_conversion_and_calculator_error_boundaries(engine, monkeypatch):
    ...
    monkeypatch.setattr(
        scipy.integrate, "dblquad", lambda callback, *_args, **_kwargs: (callback(0.0, 0.0), 0.0)
    )
    with pytest.raises(CalculatorError):
    ...
    monkeypatch.setattr(
        scipy.integrate, "tplquad", lambda callback, *_args, **_kwargs: (callback(0.0, 0.0, 0.0), 0.0)
    )
    with pytest.raises(CalculatorError):
