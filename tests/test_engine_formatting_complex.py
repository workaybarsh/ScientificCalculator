...
from scientific_calculator.engine.result_formatter import ResultFormatter
...
def test_exact_complex_format_rejects_an_invalid_cas_simplification_result(engine):
    formatter = ResultFormatter(
        engine.settings,
        to_polar=engine.to_polar,
        run_cas=lambda *_args: "not symbolic",
        require_exact_display_budget=lambda _value: None,
    )
    with pytest.raises(CalculatorError, match="karmaşık sonuç"):
        formatter.format(sp.I)
