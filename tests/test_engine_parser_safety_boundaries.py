"""Parser safety boundaries under malformed and extreme input.
...
def test_parser_budget_helpers_handle_valid_and_rejected_numeric_shapes() -> None:
    parser_module.require_exact_digit_budget(7, "unused", engine_module._PARSER_LIMITS)
    assert parser_module.estimated_factorial_digits(0) == 1
    assert parser_module.exact_nonnegative_integer(True) is None
    assert parser_module.exact_nonnegative_integer(sp.Rational(7, 2)) is None
    assert parser_module.exact_nonnegative_integer(_IntegerLike()) == 7
    assert parser_module.exact_nonnegative_integer(_BrokenInteger()) is None
...
def test_parser_numeric_helper_boundaries_are_fail_closed() -> None:
    ...
    parser_module.preflight_power(Fraction(3, 5), -4, engine_module._PARSER_LIMITS)
    parser_module.preflight_power(3, 2, engine_module._PARSER_LIMITS)

    assert parser_module.constant_numeric_approx(ast.parse("Rational(1, 2)", mode="eval").body) == 0.5
    assert parser_module.constant_numeric_approx(ast.parse("factorial(3)", mode="eval").body) == 6.0
    assert parser_module.constant_numeric_approx(ast.parse("nPr(5, 2)", mode="eval").body) == 20.0
    assert parser_module.constant_numeric_approx(ast.parse("1 / 0", mode="eval").body) == math.inf
    assert parser_module.constant_numeric_approx(ast.parse("True", mode="eval").body) is None
    assert parser_module.constant_numeric_approx(ast.parse("'not-a-number'", mode="eval").body) is None
    assert parser_module.constant_numeric_approx(ast.parse("Rational(x, 1)", mode="eval").body) is None
    assert parser_module.constant_numeric_approx(ast.parse("nCr(-1, 2)", mode="eval").body) is None
    assert parser_module.evaluated_numeric_approx(sp.Function("f")()) is None
...
def test_numeric_preflights_handle_extension_conversion_and_overflow_safely() -> None:
    ...
    with pytest.raises(CalculatorError, match="Math ERROR"):
        parser_module.preflight_power(2, 10**400, engine_module._PARSER_LIMITS)

    assert parser_module.constant_numeric_approx(ast.Constant(value=_BrokenFloatType(1))) is None
    assert parser_module.evaluated_numeric_approx(_BrokenFloat(1)) == math.inf
    assert parser_module.evaluated_numeric_approx(_NonfiniteNumeric()) == math.inf
    assert parser_module.evaluated_numeric_approx(_HugeComplexNumeric()) == math.inf
    assert parser_module.evaluated_numeric_approx(_BrokenComplexNumeric()) == math.inf
    assert parser_module.evaluated_numeric_approx(_BrokenComplexTypeNumeric()) is None
...
def test_parser_rejects_deep_or_malformed_syntax_before_evaluation(engine, monkeypatch) -> None:
    ...
    with pytest.raises(CalculatorError, match="Fonksiyona izin verilmez"):
        restricted.evaluate(ast.parse("unsafe.call()", mode="eval"))
