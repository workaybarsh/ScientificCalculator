...
def test_equals_base_complex_and_approximation_paths_use_the_engine_contract() -> None:
    ...
    complex_app._run_background_calculation.assert_called_once_with("complex_eval", ("1+i",), complex_app.show)
...
def test_auxiliary_math_dialogs_and_complex_base_operations_cover_their_user_decisions() -> None:
    ...
    app.expr = _Entry("-0.5")
    app.core.evaluate = mock.Mock(return_value=-0.5)
    ...
    app.core.evaluate.side_effect = ValueError("bad DMS")
    app.expr = _Entry("bad")
    app.err.reset_mock()
    App.dms_dialog(app)
    app.err.assert_called_once()
