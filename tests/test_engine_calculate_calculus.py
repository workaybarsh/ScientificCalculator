...
def test_numeric_integral_fallback_preserves_small_nonzero_result(engine, monkeypatch):
    ...
    monkeypatch.setattr(calculus_module, "quad", lambda *args, **kwargs: (1e-13, 1e-16))
...
def test_definite_integral_supports_convergent_generalized_intervals(
    ...
    engine.cas_isolated = False
    assert engine.definite_integral(integrand, lower, upper) == pytest.approx(expected)
...
def test_complex_and_contour_integrals_preserve_exact_complex_results(engine):
    engine.cas_isolated = False
    assert engine.complex_definite_integral("i*z", "0", "1") == sp.I / 2
    assert engine.contour_integral("1/z", "exp(i*t)", "0", "2*pi") == 2 * sp.pi * sp.I
    with pytest.raises(CalculatorError, match="tekilliği"):
        engine.contour_integral("1/z", "t", "-1", "1")
...
def test_numeric_integral_fallback_converts_integration_warning_to_error(
    ...
    monkeypatch.setattr(calculus_module, "quad", warning_quad)
    with pytest.raises(CalculatorError):
