"""Regression tests for defensive, non-UI calculator paths.
...
def test_settings_store_reports_unsupported_schema_and_unknown_migration(tmp_path) -> None:
    ...
    connection.execute("INSERT INTO meta (key, value) VALUES ('db_schema_version', '3')")
    connection.commit()
    connection.close()
    ...
    with pytest.raises(DatabaseMigrationError, match="no migration"):
        store._migrate(sqlite3.connect(":memory:"), 2)
...
def test_engine_numeric_guard_helpers_cover_exact_input_boundaries() -> None:
    ...
    assert engine_module._exact_nonnegative_integer(sp.Rational(2, 1)) == 2
    assert engine_module._exact_nonnegative_integer(sp.Integer(-1)) is None
    assert engine_module._estimated_factorial_digits(1) == 1
    assert engine_module._estimated_combinatoric_digits(4, 2, permutation=True) == 2

    assert engine_module._constant_numeric_approx(ast.parse("'1.25'", mode="eval").body) == pytest.approx(1.25)
    assert engine_module._constant_numeric_approx(ast.parse("factorial(-1)", mode="eval").body) is None
    assert engine_module._constant_numeric_approx(ast.parse("nPr(3, 2)", mode="eval").body) == pytest.approx(6)
    assert engine_module._constant_numeric_approx(ast.parse("nCr(3, 4)", mode="eval").body) is None

    assert engine_module._evaluated_numeric_approx(True) is None
    assert engine_module._evaluated_numeric_approx(np.int64(4)) == pytest.approx(4)
    assert engine_module._evaluated_numeric_approx(sp.Symbol("x")) is None
    assert engine_module._evaluated_numeric_approx(sp.nan) == math.inf
