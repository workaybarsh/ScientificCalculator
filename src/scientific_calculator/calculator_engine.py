...
def _require_exact_display_budget(value: object) -> None:
    _require_exact_digit_budget(
        value, "Math ERROR: kesin sonuç görüntüleme sınırını aşıyor"
    )


def _require_exact_resource_budget(value: object) -> None:
    """Bound exact intermediate results before later operations can amplify them."""
    _require_exact_digit_budget(
        value, "Math ERROR: kesin sonuç hesaplama sınırını aşıyor"
    )


def _require_exact_digit_budget(value: object, message: str) -> None:
...
def _preflight_power(base: object, exponent: object) -> None:
    ...
    try:
        ...
        _require_estimated_digits(max(1, digits))
...
class _RestrictedExpression:
    ...
    def evaluate(self, node: ast.AST):
        ...
        if isinstance(node, ast.BinOp):
            ...
            _require_exact_resource_budget(result)
            return result
        if isinstance(node, ast.UnaryOp):
...
class ScientificCalculatorEngine(CalculusMixin):
    ...
    def normalize(text: str) -> str:
        ...
        # Enforce the raw-input ceiling before normalisation.  Some notation
        # rewrites expand text, so checking only the transformed string lets a
        # hostile input spend disproportionate time in the normaliser.
        ...
        t = ScientificCalculatorEngine._rewrite_postfix_percent(t)
        ...
        if len(t) > _MAX_EXPRESSION_CHARS:
            raise CalculatorError("Syntax ERROR: İfade çok uzun")
        return t

    @staticmethod
    def _rewrite_postfix_percent(text: str) -> str:
        """Make ``%`` a postfix operator while preserving its operand boundary.

        Replacing every percent sign with ``/100`` changes precedence: ``1/2%``
        becomes ``(1/2)/100`` instead of ``1/(2/100)``.  Wrapping the primary
        immediately to its left gives calculator-style postfix semantics for
        numeric literals, variables, parenthesised groups, and function calls.
        """
        result = text
        while "%" in result:
            index = result.index("%")
            ...
            start = ScientificCalculatorEngine._percent_operand_start(prefix[:end])
            if start is None:
                raise CalculatorError("Syntax ERROR: yüzde için geçerli sol değer gerekli")
            operand = prefix[start:end]
            result = f"{prefix[:start]}({operand}/100){result[index + 1:]}"
        return result

    @staticmethod
    def _percent_operand_start(prefix: str) -> int | None:
        """Locate the primary expression that a postfix percent applies to."""
        end = len(prefix)
        if end == 0:
            return None
        if prefix[-1] == ")":
            ...
            for position in range(end - 1, -1, -1):
                if prefix[position] == ")":
                    ...
                    if depth == 0:
                        start = position
                        # Include a function identifier (``sin(30)%``), but
                        # not an adjacent numeric implicit multiplier.
                        name_match = re.search(r"[A-Za-z]+$", prefix[:start])
                        if name_match and name_match.group() in _ALLOWED_CALL_NAMES:
                            start = name_match.start()
                        return start
            return None
        if prefix[-1] == "!":
            base = ScientificCalculatorEngine._percent_operand_start(prefix[:-1])
            return base
        number = re.search(
            r"(?i)(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?$", prefix
        )
        if number:
            return number.start()
    ...
    def solve(self, equation: str, variable="x", guess=0.0, known_values=None):
        ...
        if residual != 0:
            slope=sp.N(self._run_cas("differentiate_at_point", {
                "expression": f, "symbol": var, "point": root,
            }),40)
    ...
    def derivative(self, expression, point, variable="x", tol=1e-10):
        ...
        try:
            exact_at=self._run_cas("differentiate_at_point", {
                "expression": expr, "symbol": x, "point": point_expr,
            })
        ...
        try:
            ...
            for _ in range(12):
                ...
                fine=(right_half-left_half)/(2*half_h)
                richardson=_finite_real_float(
                    fine+(fine-coarse)/3,
                    "Math ERROR: türev sonucu sonlu reel değil",
                )
                if abs(richardson-fine) <= tolerance*max(1.0,abs(richardson)):
                    result=richardson
                    break
                h=half_h
            if result is None:
    ...
    def one_var_stats(self,x:Iterable[float],freq:Iterable[float] | None=None):
        ...
        if freq is not None:
            ...
            if len(raw_freq)!=len(x): raise CalculatorError("Dimension ERROR")
            weights=[]
            for value in raw_freq:
                try:
                    weight=_finite_exact_integer(
                        value,"Argument ERROR: frekanslar negatif olmayan tam sayı olmalıdır"
                    )
                except CalculatorError:
                    raise
                if weight < 0 or weight > _MAX_EXACT_STATISTICS_FREQUENCY:
            ...
            if total > _MAX_EXACT_STATISTICS_FREQUENCY:
                raise CalculatorError("Argument ERROR: toplam frekans güvenli tam sayı aralığında olmalıdır")
            weight_array=np.asarray(weights,dtype=float)
            mean=float(np.sum(x*weight_array)/total)
            variance=float(np.sum(weight_array*(x-mean)**2)/total)
            sample_variance=float(np.sum(weight_array*(x-mean)**2)/(total-1)) if total>1 else float('nan')
            ...
            # and Python cumulative integers cannot wrap like int64 cumsum.
            order=np.argsort(x); values=x[order]
            cumulative=[]; running=0
            for position in order:
                running+=weights[int(position)]
                cumulative.append(running)
            def weighted_value(position):
                return float(values[bisect.bisect_left(cumulative, position)])
            ...
            result={"n":total,"Σx":float(np.sum(x*weight_array)),"Σx²":float(np.sum(x*x*weight_array)),"x̄":mean,"σx²":variance,"σx":math.sqrt(variance),
    ...
    @staticmethod
    def normal_Q(t): return stats.norm.cdf(t)-0.5
    @staticmethod
    ...
    def format_result(self,value,approximate=False):
        ...
        def _numeric_part(v):
            ...
            elif mode=="Fix": s=f"{v:.{n}f}"
            elif mode=="Sci": s=f"{v:.{max(0,n-1)}e}"
            else: s=f"{v:.12g}"
        ...
        if isinstance(value,complex):
            if abs(value.imag)<1e-12:
                ...
                # A rectangular result with no real component is clearer as
                # ``-0.886i`` than ``0-0.886i``.  This occurs frequently for
                # complex definite integrals whose exact result is purely
                # imaginary.
                if abs(value.real)<1e-12:
                    return f"{'-' if value.imag<0 else ''}{imag}i"
        ...
        if isinstance(value,sp.Basic):
            if isinstance(value, sp.Equality):
                return f"{_exact_scalar(value.lhs)} = {_exact_scalar(value.rhs)}"
            if value.has(sp.I):
                if not approximate and self.settings.input_output in ("MathI/MathO","LineI/LineO"):
                    ...
                    imag=self._run_cas("simplify", {"expression": sp.im(value)})
                    ...
                    imag_abs=self._run_cas("simplify", {"expression": imag_abs})
