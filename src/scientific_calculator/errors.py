...
# Keep longer fragments before their shorter constituents so an exact sentence
# always wins.  Every source fragment contains Turkish-specific wording, which
# means an already-English message is returned byte-for-byte unchanged.
_TURKISH_TO_ENGLISH: tuple[tuple[str, str], ...] = (
    ...
    # Integrals, multivariate calculus, line/surface operations.
    ("Karmaşık değişken ve yol parametresi farklı olmalıdır",
     "Complex variable and path parameter must be different"),
    ...
    ("kontur yalnız belirtilen değişkenleri içermelidir", "contour may contain only the specified variables"),
    ("kontur sınırları sonlu reel olmalıdır", "contour bounds must be finite and real"),
    ("kontur integrali hazırlanamadı", "contour integral could not be prepared"),
    ...
    ("çizgisel integral integrandı yalnız x ve y içermelidir", "line-integral integrand may contain only x and y"),
    ("vektör çizgisel integral hazırlanamadı", "vector line integral could not be prepared"),
    ("çizgisel integral hazırlanamadı", "line integral could not be prepared"),
    ("vektör alanı yalnız x ve y içermelidir", "vector field may contain only x and y"),
    ("yol yalnız parametre değişkenini içermelidir", "path may contain only the parameter variable"),
    ("yüzey parametreleri yalnız u ve v değişkenlerini içerebilir",
     "surface parameters may contain only u and v"),
    ("yüzey türevleri hesaplanamadı", "surface derivatives could not be computed"),
    ("yüzey integrandı yalnız x, y ve z içermelidir", "surface integrand may contain only x, y, and z"),
    ("yüzey normali yönü geçersiz", "surface-normal orientation is invalid"),
    ("akı alanı yalnız x, y ve z içermelidir", "flux field may contain only x, y, and z"),
...
class CalculatorError(Exception):
    """A calculation or input error that can be shown on the calculator LCD."""
