...
def _worker(connection: Connection, request: CalculationRequest) -> None:
    try:
        connection.send(("ok", run_calculation(request)))
    except CalculatorError as exc:
        connection.send(("input_error", str(exc)[:500]))
    except CalculationTimeout as exc:
        connection.send(("timeout", str(exc)[:500]))
    except BaseException as exc:
        connection.send(("crash", type(exc).__name__, str(exc)[:500]))
    finally:
        connection.close()
...
class CalculationController:
    ...
    def _poll_cleanup(self, process: Any, terminate: bool, attempts: int) -> None:
        ...
        try:
            ...
            if not process.is_alive():
                with suppress(OSError, ValueError):
                    process.close()
                return
            if attempts >= 20 and hasattr(process, "kill"):
                process.kill()
            elif attempts >= 40:
                self._stop_without_wait(process)
                return
            self.scheduler.after(25, lambda: self._poll_cleanup(process, terminate, attempts + 1))
        except (OSError, ValueError):
            return
