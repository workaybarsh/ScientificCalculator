...
class SpreadsheetModel:
    ...
    def _calculate_cache(self) -> dict[str, float]:
        ...
        for address in list(self.cells):
            calculated[address] = self.value(address, set())
        return calculated
    ...
    def range_values(self, start: str, end: str, stack: builtins.set[str]):
        c1, r1 = self._coords(start)
        c2, r2 = self._coords(end)
        values = []
        for row in range(min(r1, r2), max(r1, r2) + 1):
            for column in range(min(c1, c2), max(c1, c2) + 1):
                values.append(self.value(self._addr(column, row), stack))
        return values
    ...
    def value(self, address: str, stack: builtins.set[str] | None = None) -> float:
        ...
        if not raw.startswith("="):
            return float(self._evaluate(raw))

        expression = raw[1:]
        ranges = {}

        def replace_range(match):
            values = self.range_values(match.group(1), match.group(2), stack)
        ...
        for reference in sorted(references, key=len, reverse=True):
            expression = expression.replace(
                reference, str(self.value(reference, stack))
            )
        return float(self._evaluate(expression))
