...
class App(tk.Tk):
    ...
    KEYBOARD_VARIABLES=frozenset({"A", "B", "C", "D", "E", "F", "M", "x", "y", "e"})
    ...
    ERROR_TEXT_REPLACEMENTS=(
        ...
        ("İşleme izin verilmez","Operation is not permitted"),
        ("Fonksiyona izin verilmez","Function is not permitted"),
        ("Üs çok büyük","Exponent is too large"),
        ("Faktöriyel girdisi çok büyük","Factorial input is too large"),
        ("Kombinatorik girdi çok büyük","Combinatorics input is too large"),
        ...
        ("Matematiksel ifade bekleniyor","Expected a mathematical expression"),
        ("İfade yapısına izin verilmez","Expression structure is not permitted"),
        ("Özellik erişimine izin verilmez","Attribute access is not permitted"),
        ...
        ("Denklem için SOLVE kullanın; kırmızı = yalnız denklem girdisidir.","Use SOLVE for an equation; red = is for equation input only."),
        ("Bilinen değer girilmeli","A known value is required"),
        ("Başlangıç tahminini değiştirin","Change the initial guess"),
        ("hesaplama zaman aşımına uğradı","calculation timed out"),
        ("Σ aralığı hesaplama sınırını aşıyor","summation range exceeds computation budget"),
        ...
        ("seçilen boyutla bileşen sayısı eşleşmiyor","component count does not match the selected dimension"),
    )
    def __init__(self):
        super().__init__()
        try:
    ...
    def _db_base_path(self):
        ...
        folder = os.path.join(root, "ScientificCalculator")
        os.makedirs(folder, exist_ok=True)
        return os.path.join(folder, "settings.db")
    ...
    def _migrate_settings(cls, saved):
        ...
        payload=dict(saved)
        if version < 2:
            # v1 already used the final field names; v2 formalizes the schema
            # pipeline and preserves all valid values through sanitization.
    ...
    def _format_error_message(self,e):
        message=str(e)
        for turkish,english in self.ERROR_TEXT_REPLACEMENTS:
    ...
    def _lcd_field_text(self,flow,spec):
        ...
        if spec.get("type")=="choice" and default in spec.get("choices",{}).values():
            for number,choice in spec["choices"].items():
                if choice==default:
                    return str(number)
    ...
    def _lcd_start_equation(self):
        kinds={1:"Polynomial",2:"Simultaneous"}
    ...
    def _lcd_sheet_tools(self):
        tools={
            ...
            6:"Fill formula",7:"Recalculate",8:"Free space",9:"Delete all",
        }
    ...
    def start_integral_template(self):
        ...
        self.template_fields={"lower":"","upper":"","body":current,"var":"x"}
        self.template_order=["body","upper","lower"]
        self.template_index=self.template_order.index("body")
        self.template_cursors={k:len(v) for k,v in self.template_fields.items()}
        self.render_template()
        self.result.config(text="∫  RAD  •  ▲ upper  •  ▼ lower")
        ...
        if kind=="integral":
            ...
            if lo and hi:
                self._run_background_calculation(
                    "definite_integral", (body,lo,hi,var),
    ...
    def _open_workspace_window(self,key,title):
        """Open one visible workspace per mode and restore main-input focus on close."""
        workspaces=self.__dict__.get("_workspaces")
        if workspaces is None:
            workspaces={}; self._workspaces=workspaces
        ...
        w.title(title)
        try:
            w.transient(self); w.lift(); w.after_idle(w.focus_set)
        except tk.TclError:
            pass

        def close():
            if workspaces.get(key) is w:
            ...
            try:
                ...
                try:
                    if self.winfo_exists():
                        self.after_idle(self.expr.focus_set)
                except tk.TclError:
                    pass

        w.protocol("WM_DELETE_WINDOW",close)
        w.bind("<Escape>",lambda _event:(close(),"break")[1])
        return w,True

    def setup_dialog(self):
        w=tk.Toplevel(self); w.title("SETUP"); w.geometry("460x730"); w.resizable(False,False)
        vars={}
        ...
        ttk.Button(buttons,text="Save",command=save).pack(side="left",expand=True,fill="x",padx=(0,6))
        ttk.Button(buttons,text="Reset to Defaults",command=reset).pack(side="left",expand=True,fill="x",padx=(6,0))
    ...
    def eng_key(self):
        ...
        try:
            v=float(sp.N(self.core.ans)); exp=0 if v==0 else int(math.floor(math.log10(abs(v))/3)*3); exp += (-3 if self.shift else 3); self.result.config(text=f"{v/(10**exp):.10g}×10^{exp}")
        except Exception as e:self.err(e)
        self.consume()
    def lparen_key(self):
    ...
    def integral_key(self):
        ...
        self.consume(); self.start_integral_template()
    ...
    def optn_key(self):
        ...
        if self.mode=="Calculate":
            ...
            menu.add_separator(); menu.add_command(label="→r∠θ",command=self.complex_to_polar); menu.add_command(label="→a+bi",command=self.complex_from_polar)
    ...
    def constants_dialog(self):
        w=tk.Toplevel(self); w.title("CONST – 47 Scientific Constants (CODATA 2010 legacy)"); lb=tk.Listbox(w,width=65,height=22); lb.pack(padx=8,pady=8,fill="both",expand=True)
        ttk.Label(w,text=CONSTANTS_DATASET_LABEL).pack(padx=8,pady=(6,0))
        keys=list(CONSTANTS)
    ...
    def mode_workspace(self,m):
        if m in self.LCD_WORKSPACE_MODES:
            self._start_lcd_flow(m)
            return
        if m=="Matrix":self.matrix_dialog()
        elif m=="Vector":self.vector_dialog()
        elif m=="Statistics":self.statistics_dialog()
        elif m=="Distribution":self.distribution_dialog()
        elif m=="Table":self.table_dialog()
        elif m=="Equation/Func":self.equation_dialog()
        elif m=="Inequality":self.inequality_dialog()
        elif m=="Ratio":self.ratio_dialog()
        elif m=="Spreadsheet":self.spreadsheet_dialog()

    def _array_input(self,title,rows,cols):
        s=simpledialog.askstring(title,f"Enter {rows}×{cols} values row by row; separate with commas or spaces:",parent=self)
        if s is None:return None
        vals=[float(x) for x in re.split(r'[ ,;]+',s.strip()) if x]
        if len(vals)!=rows*cols:raise CalculatorError("Dimension ERROR: incorrect value count")
        return np.array(vals).reshape(rows,cols)
    def matrix_dialog(self):
        w,created=self._open_workspace_window("matrix","Matrix")
        if not created:return
        out=tk.Text(w,width=60,height=15); out.pack(padx=8,pady=8)
        def define():
            n=simpledialog.askstring("Define Matrix","MatA / MatB / MatC / MatD",parent=w)
            if not n:return
            r=simpledialog.askinteger("Rows","1–4",minvalue=1,maxvalue=4,parent=w)
            if r is None:return
            c=simpledialog.askinteger("Columns","1–4",minvalue=1,maxvalue=4,parent=w)
            if c is None:return
            try:
                a=self._array_input(n,r,c)
                if a is None:return
                self.core.define_matrix(n,a); out.insert(tk.END,f"{n}=\n{a}\n")
            except Exception as e:self.err(e)
        ttk.Button(w,text="Define/Edit Matrix",command=define).pack(fill="x",padx=8)
        ops=ttk.Frame(w); ops.pack(fill="x",padx=8,pady=4)
        for label,op in [("A+B","+"),("A-B","-"),("A×B","*"),("det(A)","det"),("A⁻¹","inv"),("Trn(A)","trn"),("A²","square"),("A³","cube"),("Abs(A)","abs")]:
            def go(o=op):
                try:
                    a=simpledialog.askstring("Matrix","First matrix",initialvalue="MatA",parent=w); b=None
                    if a is None:return
                    if o in "+-*": b=simpledialog.askstring("Matrix","Second matrix",initialvalue="MatB",parent=w)
                    if o in "+-*" and b is None:return
                    r=self.core.matrix_op(o,a,b); out.insert(tk.END,f"{o}:\n{r}\n")
                except Exception as e:self.err(e)
            ttk.Button(ops,text=label,command=go).pack(side="left",expand=True,fill="x")
        ttk.Button(w,text="Identity",command=lambda:self._identity_ui(out,w)).pack(fill="x",padx=8,pady=3)
        ttk.Button(w,text="Copy → MatA-D",command=lambda:self.matrix_copy_ui(out,w)).pack(fill="x",padx=8,pady=3)
    def _identity_ui(self,out,w):
        try:
            n=simpledialog.askinteger("Identity","Size 1–4",minvalue=1,maxvalue=4,parent=w)
            if n is None:return
            out.insert(tk.END,str(self.core.identity(n))+"\n")
        except Exception as e:self.err(e)
    def matrix_copy_ui(self,out,w):
        src=simpledialog.askstring("Copy","Source MatA/MatB/MatC/MatD/MatAns",parent=w); dst=simpledialog.askstring("Copy","Destination MatA/MatB/MatC/MatD",parent=w)
        if src is None or dst is None:return
        try:
            A=self.core.mat_ans if src=="MatAns" else self.core.matrices.get(src)
            if A is None or dst not in self.core.matrices: raise CalculatorError("Dimension ERROR")
            self.core.matrices[dst]=np.array(A,copy=True); out.insert(tk.END,f"{src} → {dst}\n")
        except Exception as e:self.err(e)
    def vector_dialog(self):
        w,created=self._open_workspace_window("vector","Vector")
        if not created:return
        out=tk.Text(w,width=60,height=14); out.pack(padx=8,pady=8)
        def define():
            n=simpledialog.askstring("Define Vector","VctA / VctB / VctC / VctD",parent=w)
            if not n:return
            d=simpledialog.askinteger("Dimension","2 or 3",minvalue=2,maxvalue=3,parent=w)
            if d is None:return
            s=simpledialog.askstring(n,"Components, comma-separated",parent=w)
            if s is None:return
            try:
                if n not in self.core.vectors:raise CalculatorError("Argument ERROR: invalid vector name")
                a=[float(x) for x in re.split(r'[ ,;]+',s) if x]
                if len(a)!=d:raise CalculatorError("Dimension ERROR: component count does not match the selected dimension")
                self.core.define_vector(n,a); out.insert(tk.END,f"{n}={a}\n")
            except Exception as e:self.err(e)
        ttk.Button(w,text="Define/Edit Vector",command=define).pack(fill="x",padx=8)
        for label,op in [("A+B","+"),("A-B","-"),("Dot","dot"),("Cross","cross"),("Angle","angle"),("Abs","abs"),("Unit Vector","unit")]:
            def go(o=op):
                try:
                    a=simpledialog.askstring("Vector","First vector",initialvalue="VctA",parent=w); b=None
                    if a is None:return
                    if o in ("+","-","dot","cross","angle"):b=simpledialog.askstring("Vector","Second vector",initialvalue="VctB",parent=w)
                    if o in ("+","-","dot","cross","angle") and b is None:return
                    r=self.core.vector_op(o,a,b); out.insert(tk.END,f"{o}: {r}\n")
                except Exception as e:self.err(e)
            ttk.Button(w,text=label,command=go).pack(fill="x",padx=8,pady=1)
        ttk.Button(w,text="Scalar × Vector",command=lambda:self.vector_scale_ui(out,w)).pack(fill="x",padx=8,pady=1)
        ttk.Button(w,text="Copy → VctA-D",command=lambda:self.vector_copy_ui(out,w)).pack(fill="x",padx=8,pady=1)
    def vector_scale_ui(self,out,w):
        a=simpledialog.askstring("Vector","Vector",initialvalue="VctA",parent=w); k=simpledialog.askfloat("Vector","Scalar",parent=w)
        if a is None or k is None:return
        try:out.insert(tk.END,f"{k}×{a} = {self.core.vector_op('scale',a,scalar=k)}\n")
        except Exception as e:self.err(e)
    def vector_copy_ui(self,out,w):
        src=simpledialog.askstring("Copy","Source VctA/VctB/VctC/VctD/VctAns",parent=w); dst=simpledialog.askstring("Copy","Destination VctA/VctB/VctC/VctD",parent=w)
        if src is None or dst is None:return
        try:
            A=self.core.vct_ans if src=="VctAns" else self.core.vectors.get(src)
            if A is None or dst not in self.core.vectors: raise CalculatorError("Dimension ERROR")
            self.core.vectors[dst]=np.array(A,copy=True); out.insert(tk.END,f"{src} → {dst}\n")
        except Exception as e:self.err(e)
    def _nums(self,s):return [float(x) for x in re.split(r'[ ,;\n]+',s.strip()) if x]

    @staticmethod
    def _spreadsheet_data_column_index(column):
        """Map a Treeview column token to A–E, excluding the row-number column."""
        if not isinstance(column,str):
            return None
        match=re.fullmatch(r"#([1-6])",column)
    ...
    def statistics_dialog(self):
        w,created=self._open_workspace_window("statistics","Statistics")
        if not created:return
        typ=tk.StringVar(value="1-Variable"); ttk.Combobox(w,textvariable=typ,values=["1-Variable","Linear","Quadratic","Logarithmic","e Exponential","ab Exponential","Power","Inverse"],state="readonly").pack(fill="x",padx=8,pady=4)
        ttk.Label(w,text="x data").pack(); x=tk.Text(w,width=55,height=4); x.pack(padx=8); ttk.Label(w,text="y data / frequencies when enabled in SETUP").pack(); y=tk.Text(w,width=55,height=4); y.pack(padx=8); out=tk.Text(w,width=60,height=14); out.pack(padx=8,pady=5)
        def calc():
            try:
                xs=self._nums(x.get('1.0','end'))
                if typ.get()=="1-Variable":
                    freq=None
                    if self.core.settings.statistics_freq:
                        freq=self._nums(y.get('1.0','end'))
                        if not freq:raise CalculatorError("Argument ERROR: frequency data is required")
                    r=self.core.one_var_stats(xs,freq); out.delete('1.0','end'); out.insert('end','\n'.join(f'{k} = {v}' for k,v in r.items()))
                else:
                    ys=self._nums(y.get('1.0','end')); km={"Linear":"linear","Quadratic":"quadratic","Logarithmic":"log","e Exponential":"exp_e","ab Exponential":"exp_b","Power":"power","Inverse":"inverse"}; r=self.core.regression(xs,ys,km[typ.get()]); out.delete('1.0','end'); out.insert('end','\n'.join(f'{k} = {"undefined" if v is None else v}' for k,v in r.items() if k!='predict'))
            except Exception as e:self.err(e)
        ttk.Button(w,text="Calculate",command=calc).pack(pady=4)
        aux=ttk.Frame(w); aux.pack(fill="x",padx=8,pady=3)
        for label,fn in [("P(t)",self.core.normal_P),("Q(t)",self.core.normal_Q),("R(t)",self.core.normal_R)]:
            ttk.Button(aux,text=label,command=lambda f=fn,l=label:self.stat_normal_aux(f,l,w)).pack(side="left",expand=True,fill="x")
        ttk.Button(aux,text="x→t",command=lambda:self.stat_xt_ui(x,y,w)).pack(side="left",expand=True,fill="x")
    def stat_normal_aux(self,fn,label,w):
        t=simpledialog.askfloat(label,"t =",parent=w)
        if t is not None:self.result.config(text=f"{label} = {fn(t):.12g}")
    def stat_xt_ui(self,xwidget,ywidget,w):
        try:
            xs=self._nums(xwidget.get('1.0','end'))
            freq=None
            if self.core.settings.statistics_freq:
                freq=self._nums(ywidget.get('1.0','end'))
                if not freq:raise CalculatorError("Argument ERROR: frequency data is required")
            st=self.core.one_var_stats(xs,freq); xv=simpledialog.askfloat("x→t","x =",parent=w)
            if xv is not None:self.result.config(text=f"t = {(xv-st['x̄'])/st['σx']:.12g}")
        except Exception as e:self.err(e)
    def distribution_dialog(self):
        w,created=self._open_workspace_window("distribution","Distribution")
        if not created:return
        kinds=["Normal PD","Normal CD","Inverse Normal","Binomial PD","Binomial CD","Poisson PD","Poisson CD"]
        output=tk.StringVar(value="Choose a distribution and enter its values.")
        for k in kinds:
            def go(kind=k):
                try:
                    kw={}
                    fields={"Normal PD":["x","sigma","mu"],"Normal CD":["lower","upper","sigma","mu"],"Inverse Normal":["area","sigma","mu"],"Binomial PD":["x","N","p"],"Binomial CD":["x","N","p"],"Poisson PD":["x","lam"],"Poisson CD":["x","lam"]}[kind]
                    for f in fields:
                        v=simpledialog.askfloat(kind,f,parent=w)
                        if v is None:return
                        kw[f]=v
                    r=self.core.distribution(kind,**kw)
                    text=f"{kind} = {r:.12g}"
                    self.result.config(text=text); output.set(text)
                except Exception as e:self.err(e)
            ttk.Button(w,text=k,command=go).pack(fill="x",padx=10,pady=2)
        ttk.Label(w,textvariable=output,anchor="w",wraplength=420).pack(fill="x",padx=10,pady=(6,10))
    def table_dialog(self):
        # A visible table is its own workspace.  Focus it instead of collecting
        # a new set of values that cannot be shown while it is already open.
        existing=self.__dict__.get("_workspaces",{}).get("table")
        try:
            if existing is not None and existing.winfo_exists():
                existing.deiconify(); existing.lift(); existing.focus_set()
                return
            ...
            pass
        f=simpledialog.askstring("Table","f(x)=",initialvalue="x^2",parent=self)
        if f is None:return
        g=None
        if self.core.settings.table_two_functions:
            g=simpledialog.askstring("Table","g(x)=",initialvalue="x^2+1",parent=self)
            if g is None:return
        a=simpledialog.askfloat("Table","Start",initialvalue=-1,parent=self)
        if a is None:return
        b=simpledialog.askfloat("Table","End",initialvalue=1,parent=self)
        if b is None:return
        h=simpledialog.askfloat("Table","Step",initialvalue=.5,parent=self)
        if h is None:return
        try:n=self._table_row_count(a,b,h,g is not None)
        except CalculatorError as e:self.err(e);return
        w,created=self._open_workspace_window("table","Table")
        if not created:return
        tree=ttk.Treeview(w,columns=("x","f","g") if g else ("x","f"),show="headings")
        for c in tree['columns']:tree.heading(c,text=c); tree.column(c,width=120)
        scroll=ttk.Scrollbar(w,orient="vertical",command=tree.yview)
        tree.configure(yscrollcommand=scroll.set)
        tree.pack(side="left",fill="both",expand=True)
        scroll.pack(side="right",fill="y")
        try:
            for i in range(n):
                xv=a+i*h; fv=self.core.parse(f,{"x":sp.Float(xv)}); row=[xv,float(sp.N(fv))]
                if g:row.append(float(sp.N(self.core.parse(g,{"x":sp.Float(xv)}))))
                tree.insert('',tk.END,values=row)
            self.core.memory['x']=sp.Float(a+(n-1)*h)
        except Exception as e:self.err(e)
    def equation_dialog(self):
        w,created=self._open_workspace_window("equation","Equation/Func")
        if not created:return
        ttk.Button(w,text="Simul Equation 2–4 unknowns",command=lambda:self.simul_ui(w)).pack(fill="x",padx=10,pady=4)
        ttk.Button(w,text="Polynomial 2nd–4th degree",command=lambda:self.poly_ui(w)).pack(fill="x",padx=10,pady=4)
    def simul_ui(self,w):
        n=simpledialog.askinteger("Simul Equation","Number of unknowns 2–4",minvalue=2,maxvalue=4,parent=w)
        if not n:return
        try:
            A=self._array_input("Coefficient matrix",n,n)
            if A is None:return
            b=self._array_input("Constants",n,1)
            if b is None:return
            r=self.core.simultaneous(A,b.reshape(n)); messagebox.showinfo("Solution","\n".join(f"x{i+1} = {v:.12g}" for i,v in enumerate(r)),parent=w)
        except Exception as e:self.err(e)
    def poly_ui(self,w):
        n=simpledialog.askinteger("Polynomial","Degree 2–4",minvalue=2,maxvalue=4,parent=w)
        if n is None:return
        s=simpledialog.askstring("Polynomial",f"Enter exactly {n+1} coefficients as a,b,c...",parent=w)
        if s is None:return
        try:
            coeff=self._nums(s)
            if len(coeff)!=n+1:raise CalculatorError(f"Argument ERROR: degree {n} requires exactly {n+1} coefficients")
            roots=self.core.polynomial_roots(coeff); msg='\n'.join(f"x{i+1} = {r}" for i,r in enumerate(roots))
            if n==2:
                a,b,c=coeff; xv=-b/(2*a); yv=a*xv*xv+b*xv+c; msg+=f"\nVertex: ({xv:.12g}, {yv:.12g})"
            messagebox.showinfo("Roots",msg,parent=w)
        except Exception as e:self.err(e)
    def inequality_dialog(self):
        n=simpledialog.askinteger("Inequality","Degree 2–4",minvalue=2,maxvalue=4,parent=self)
        if n is None:return
        s=simpledialog.askstring("Inequality",f"Enter exactly {n+1} coefficients",parent=self)
        if s is None:return
        rel=simpledialog.askstring("Inequality","Relation: >, <, ≥, ≤",initialvalue=">",parent=self)
        if rel is None:return
        try:
            coeff=self._nums(s)
            if len(coeff)!=n+1:raise CalculatorError(f"Argument ERROR: degree {n} requires exactly {n+1} coefficients")
            r=self.core.inequality(coeff,rel); messagebox.showinfo("Solution",str(r),parent=self)
        except Exception as e:self.err(e)
    def ratio_dialog(self):
        kind=simpledialog.askstring("Ratio","A:B=X:D or A:B=C:X",initialvalue="A:B=X:D",parent=self)
        if kind is None:return
        try:
            if kind=="A:B=X:D":
                A=simpledialog.askfloat("Ratio","A",parent=self)
                if A is None:return
                B=simpledialog.askfloat("Ratio","B",parent=self)
                if B is None:return
                D=simpledialog.askfloat("Ratio","D",parent=self)
                if D is None:return
                r=self.core.ratio(kind,A=A,B=B,D=D)
            elif kind=="A:B=C:X":
                A=simpledialog.askfloat("Ratio","A",parent=self)
                if A is None:return
                B=simpledialog.askfloat("Ratio","B",parent=self)
                if B is None:return
                C=simpledialog.askfloat("Ratio","C",parent=self)
                if C is None:return
                r=self.core.ratio("A:B=C:X",A=A,B=B,C=C)
            else:
                raise CalculatorError("Argument ERROR: unsupported ratio form")
            self.result.config(text=f"X = {r:.12g}")
        except Exception as e:self.err(e)
    def spreadsheet_dialog(self):
        w,created=self._open_workspace_window("spreadsheet","Spreadsheet A1:E45")
        if not created:return
        top=ttk.Frame(w); top.pack(fill="x"); addr=tk.StringVar(value="A1"); entry=tk.StringVar()
        ttk.Entry(top,textvariable=addr,width=6).pack(side="left",padx=2); ttk.Entry(top,textvariable=entry).pack(side="left",fill="x",expand=True,padx=2)
        tree=ttk.Treeview(w,columns=["#"]+list("ABCDE"),show="headings",height=20); tree.heading("#",text=""); tree.column("#",width=35)
        for c in "ABCDE":tree.heading(c,text=c); tree.column(c,width=90)
        tree.pack(fill="both",expand=True,padx=5,pady=5)
        selected_column={"index":1}

        stale_status=tk.StringVar(value="")
        def refresh(recalculate=False):
            for i in tree.get_children():tree.delete(i)
            if recalculate:
                try:self.sheet.recalculate()
                except Exception as e:self.err(e)
            for r in range(1,46):tree.insert('',tk.END,iid=str(r),values=[r]+[self._spreadsheet_display_value(f'{c}{r}') for c in 'ABCDE'])
            stale_status.set("Recalculation required — stale values are marked *" if getattr(self.sheet,"needs_recalculation",False) else "")
        def setcell():
            try:self.sheet.set(addr.get(),entry.get()); refresh()
            except Exception as e:self.err(e)
        def delete_cell():
            try:self.sheet.delete(addr.get()); refresh()
            except Exception as e:self.err(e)
        def remember_column(event):
            column=self._spreadsheet_data_column_index(tree.identify_column(event.x))
            if column is not None:
                selected_column["index"]=column
        def select(ev=None):
            sel=tree.selection()
            if not sel:return
            ci=selected_column["index"]
            a=f"{'ABCDE'[ci-1]}{sel[0]}"; addr.set(a); entry.set(str(self._spreadsheet_display_value(a)))
        tree.bind('<Button-1>',remember_column,add='+')
        tree.bind('<<TreeviewSelect>>',select); ttk.Button(top,text="=",command=setcell).pack(side="left")
        bar=ttk.Frame(w); bar.pack(fill="x")
        ttk.Button(bar,text="Delete",command=delete_cell).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Delete All",command=lambda:(self.sheet.delete_all(),refresh())).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Copy & Paste",command=lambda:self.sheet_copy_ui(False,refresh,w)).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Cut & Paste",command=lambda:self.sheet_copy_ui(True,refresh,w)).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Fill",command=lambda:self.sheet_fill_ui(refresh,w)).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Free Space",command=lambda:messagebox.showinfo("Free Space",f"{self.sheet.free_space()} bytes",parent=w)).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Recalculate",command=lambda:refresh(True)).pack(side="left",expand=True,fill="x")
        ttk.Button(bar,text="Grab",command=lambda:entry.set(entry.get()+addr.get())).pack(side="left",expand=True,fill="x")
        ttk.Label(w,textvariable=stale_status,anchor="w").pack(fill="x",padx=6,pady=(0,4))
        refresh()
    def sheet_copy_ui(self,cut,refresh,w):
        s=simpledialog.askstring("Spreadsheet","Source cell",parent=w); d=simpledialog.askstring("Spreadsheet","Destination cell",parent=w)
        if s is None or d is None:return
        try:(self.sheet.cut if cut else self.sheet.copy)(s,d); refresh()
        except Exception as e:self.err(e)
    def sheet_fill_ui(self,refresh,w):
        s=simpledialog.askstring("Fill","Starting cell",parent=w); e=simpledialog.askstring("Fill","Ending cell",parent=w); v=simpledialog.askstring("Fill","Value or =formula",parent=w)
        if s is None or e is None or v is None:return
        try:
            if v.startswith('='):self.sheet.fill_formula(s,e,v)
            else:self.sheet.fill_value(s,e,v)
            refresh()
        except Exception as ex:self.err(ex)

    def help_key(self):
        messagebox.showinfo("Quick help","MENU: 12 modes\nSHIFT+MENU: SETUP\nLCD modes: = next/run • ▲▼ field/result • ◀▶ choice/cell • OPTN action • AC restart\nALPHA+CALC: red =\nSHIFT+CALC: SOLVE\n∫: integral template; ▲ upper bound, ▼ lower bound\n  ◀/▶ expression/variable only • empty bounds: symbolic +C\nSHIFT+∫: derivative template; empty point: symbolic\nSHIFT+x: Σ\nSHIFT+7: CONST\nSHIFT+8: CONV\nSHIFT+9: RESET",parent=self)
