...
`∫` tuşu integral şablonunu açar.

1. Fonksiyonu yazın.
2. `▲` ile üst sınıra, `▼` ile alt sınıra geçin.
3. `◀` / `▶` ile fonksiyon alanı ve değişken alanı arasında gezin.
4. `=` ile hesaplayın.
...
İki sınırı da boş bırakırsanız sembolik integral ve `+ C` sonucu alınır:

```text
∫ x^2 dx  → x^3/3 + C
```

Yalnız bir sınır girmek geçerli değildir. Tekillik veya yakınsamayan integral varsa uygulama Math ERROR gösterir.

### Türev

`SHIFT + ∫` türev şablonunu açar.

- Fonksiyonu girin.
- Nokta alanını boş bırakırsanız sembolik türev alınır.
- Nokta girerseniz o noktadaki sayısal türev hesaplanır.
...
- **Grab**: Seçili hücre adresini girişe ekleme
...
**Save** ayarları yerel Windows kullanıcı profilinde uygulamanın SQLite veritabanında saklar. Aynı veritabanı son 10 hesaplamanın ifadesini ve ekranda gösterilen sonucunu da tutar. Uygulamayı pencerenin kapatma düğmesiyle veya `SHIFT + AC` ile kapatmak da geçerli ayarları kaydeder.
...
Hesaplama, giriş ve ayar hataları ayrı bir hata penceresi açmadan doğrudan calculator LCD'sinde gösterilir. Aktif giriş temizlenir, mod korunur; `Ans` ve History son başarılı değerlerinde kalır.
