...
Scientific Calculator; Windows, Linux ve macOS için çevrimdışı çalışan bağımsız bir hesaplama aracıdır. Klasik bilimsel hesap makinesi yerleşiminden yararlanır; bir emülatör, firmware kopyası veya herhangi bir üreticiyle bağlantılı ürün değildir.

Bu kılavuz, ilk hesaplamadan gelişmiş modlara kadar günlük kullanım için hazırlanmıştır.

Güncel Graphite görünümü, LCD düzeni ve tuş eşlemesi için [Arayüz Turu](docs/INTERFACE.md) belgesine bakın. Doğru platform paketini seçme, sağlama toplamını doğrulama, kurulum, taşınabilir kullanım ve tüm verileriyle kaldırma adımları [Kurulum, çalıştırma ve kaldırma](docs/INSTALLATION.md) belgesindedir.

## İçindekiler

1. [Kurulum, çalıştırma ve kaldırma](docs/INSTALLATION.md)
2. [Arayüz Turu](docs/INTERFACE.md)
3. [Hızlı başlangıç](#hızlı-başlangıç)
4. [Temel tuşlar ve yazım](#temel-tuşlar-ve-yazım)
5. [Calculate modu](#calculate-modu)
6. [İntegral, türev, toplam ve SOLVE](#integral-türev-toplam-ve-solve)
7. [Sonuç biçimi, bellek ve geçmiş](#sonuç-biçimi-bellek-ve-geçmiş)
8. [Özel hesap modları](#özel-hesap-modları)
9. [Spreadsheet, Table ve Equation araçları](#spreadsheet-table-ve-equation-araçları)
10. [SETUP, skin ve kayıt](#setup-skin-ve-kayıt)
11. [Hata mesajları ve sorun giderme](#hata-mesajları-ve-sorun-giderme)

---

## Hızlı başlangıç

1. Platformunuza ait paketi kurun veya açın, ardından uygulamayı başlatın.
...
- `=` sonrasında ilk `▲`, girilmiş ifadeyi düzenlemek için geri çağırır. Sonraki `▲` / `▼` geçmişte gezinir; `AC` sonrasında ilk `▲` her zaman en yeni kaydı açar. `MENU` → `History`, son 10 ifade ve tam sonucu ayrı pencere açmadan LCD üzerinde gösterir; `▲` / `▼` ile satır seçin, uzun satırı `◀` / `▶` ile kaydırın ve ham ifadeyi yeniden hesaplamadan çağırmak için `OPTN` kullanın.
...
Hesap makinesi integral, türev, toplam ve SOLVE dahil son 10 başarılı hesaplamayı ve ekranda gösterilen sonuçlarını yerel olarak saklar. `MENU` → `History` seçildiğinde liste ayrı pencere açmadan mevcut LCD’de görünür; en yeni kayıt `1`, ondan önceki kayıt `2` olarak başlar. Her satırda tam işlem, `=` ve sonuç korunur; `▲` / `▼` ile kayıtlar arasında gezinilir, uzun satırlar `◀` / `▶` ile incelenir, seçili ifade yeniden hesaplamadan `OPTN` ile çağrılır ve `AC` ile normal hesap ekranına dönülür. **Reset to Defaults**, Setup ayarlarıyla birlikte bu geçmişi de siler. İntegral girişinde `sinxcosx`, `sin(x)×cos(x)` olarak kabul edilir.
...
**Calculate** modunda `∫`, ayrı bir pencere açmadan yalnız LCD üzerinde çalışan işlem seçiciyi açar. Seçicide **Integral**, **Double** ve **Triple** bulunur. `SHIFT + ∫` ayrı türev şablonunu açar. `SHIFT + OPTN`, görünür `∞` sınır işaretini ekler.

**Integral** seçildiğinde alışılmış hesap makinesi şablonu görünür:

1. Fonksiyonu yazın.
2. `▲` ile üst sınıra, `▼` ile alt sınıra geçin.
3. `d` yanındaki boş alana `TAB` veya `◀` / `▶` ile gelip diferansiyel değişkenini yazın.
4. `=` ile hesaplayın.
...
Her iki sınırı boş bırakırsanız sembolik integral ve `+ C` sonucu alınır:

```text
∫ x^2 dx  → x^3/3 + C
```

Çok katlı integral şablonlarında bütün sınırlar ve `d□` değişkenleri boş gelir; hesap için hepsini doldurun. İç sınır, dış integral değişkenini içeren `x`, `x^2` veya `sin(x)` gibi bir ifade olabilir.

### Türev

`SHIFT + ∫` hem Calculate hem Complex modunda türev şablonunu açar: nokta alanı boşsa türev fonksiyonu, girilirse o noktadaki türev hesaplanır. History kaydı integral veya türevin tamamını ve sonucunu birlikte saklar.
...
**Complex** modunda `∫`, `OPTN` veya ayrı pencere kullanmadan LCD üzerindeki karmaşık integral şablonunu açar. İki sınır girilirse karmaşık belirli integral, iki sınır da boşsa `+ C` ile sembolik integral hesaplanır. Türev için `SHIFT + ∫` kullanılır: nokta boşsa türev fonksiyonu, nokta girilirse o noktadaki türev gösterilir. History, integral veya türevin tamamını sonucu ile birlikte saklar.
...
Matrix LCD formunda `MatA`–`MatD` tanımlanabilir; boyut sınırı 1×1 ile 4×4 arasındadır. Her satırı tek seferde girip `=` ile onaylayın. Satırda boşluk, virgül, noktalı virgül, `+` ve eksi işareti değer ayırıcıdır: `1+2+3`, `1, 2, 3` olarak; `1-2+3`, `1, -2, 3` olarak; `-1-1-1` ise `-1, -1, -1` olarak kaydedilir. Üst klavyedeki `-` ile hesap makinesi/numpad’deki `−` aynı şekilde kabul edilir; bilimsel gösterimdeki üs işaretleri de korunur.
...
- **Simul Equation**: 2–4 bilinmeyenli doğrusal denklem sistemi çözümü. `=` tamamlanan satırı kaydedip bir sonraki satıra geçirir; son satırdan sonra `x1`, `x2`, … sonuçları ayrı satırlarda görünür ve `▲` / `▼` ile gezilir. 4×4 sistemde alanlar iç içe geçmesin diye denklem iki satıra bölünür; katsayı ve sağ taraf kutuları `◀` / `▶` ile gezilir.
- **Polynomial**: 2. veya 3. derece polinom kökleri; ikinci derecede tepe noktası da gösterilir. Katsayılar ayrı şablon kutularına soldan sağa girilir; sonuçta `x1`, `x2`, … satırları `▲` / `▼` ile gezilir.
- **Differential Eq.**: İkinci dereceden lineer sembolik adi diferansiyel denklem çözümü. Ekran doğrudan `□·y'' + □·y' + □·y = □` biçimindedir ve okunaklı kalması için iki satıra bölünür; dört katsayı kutusuna soldan sağa `1`, `x`, `sin(x)` ya da `0` gibi ifadeleri girin. Uzun ifadeler kutu içinde `…` ile görünür; `◀` / `▶` ile kutular arasında geçip `=` ile çözün. Uygulama oluşan lineer denklemi otomatik belirler. PDE'ler, üçüncü ve daha yüksek dereceden ODE'ler ve ikinci dereceden doğrusal olmayan denklemler açıkça desteklenmez.

Örneğin `x²-5x+6` için üç polinom kutusuna sırayla `1`, `-5`, `6` girin.
...
| UI Scale | 40, 50, 60, 75, 100, 125, 150 veya 200% |
...
**Save** ayarları geçerli kullanıcının SQLite veritabanında saklar. Windows’ta normal yol `%LOCALAPPDATA%\ScientificCalculator\settings.db`’dir; `LOCALAPPDATA` yoksa Windows, Linux ve macOS’ta `~/.scientific_calculator/ScientificCalculator/settings.db` kullanılır. Aynı veritabanı son 10 hesaplamanın ifadesini ve ekranda gösterilen sonucunu da tutar. Uygulamayı pencerenin kapatma düğmesiyle veya `SHIFT + AC` ile kapatmak da geçerli ayarları kaydeder. Varsayılan ölçek tüm platformlarda %100’dür; macOS yalnız gerçek pencere alanı skin’i sığdıramazsa etkili ölçeği kontrollü düşürür. Yerel kurulum kaldırıcıları açık Scientific Calculator sürecini önce kapatır, sonra yalnız bu sabit uygulama-verisi yolunu siler; macOS’ta tam sıfırlama için kurulumla gelen **Scientific Calculator Uninstaller.app** açılır.
...
Hesaplama, giriş ve ayar hataları ayrı bir hata penceresi açmadan doğrudan calculator LCD'sinde gösterilir. Aktif giriş temizlenir, mod korunur; `Ans` ve History son başarılı değerlerinde kalır. İntegral ve türev şablonlarındaki hatalar matematik düzeni bozulmasın diye LCD'nin sağ altında tek, kompakt bir satırda gösterilir. Kullanıcıya görünen tüm hata metinleri İngilizcedir; eski motor metinleri LCD'ye aktarılmadan önce çevrilir.
...
Yayın dosyalarını her zaman resmi GitHub release sayfasından indirin. İndirdiğiniz Windows, Linux veya macOS paketini açmadan önce `SHA256SUMS.txt` ile doğrulayın. Paket adları ve her platform için ayrıntılı kurulum/kaldırma adımları [Kurulum, çalıştırma ve kaldırma](docs/INSTALLATION.md) belgesindedir.
