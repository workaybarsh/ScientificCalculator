...
In **Calculate** mode, press `∫` to open the LCD-only calculus chooser; no dialog window opens. Use `◀` / `▶` to choose an action and `=` to advance through its fields or run it. The chooser includes definite and indefinite integrals, symbolic derivatives, double/triple integrals, scalar and vector line integrals, and scalar surface and flux integrals.

Choose **Definite Integral** for the familiar on-calculator template:

1. Enter the function.
2. Use `▲` for the upper bound and `▼` for the lower bound.
3. Use `TAB` to reach the differential-variable field.
...
Choose **Indefinite Integral** for a symbolic result, displayed with `+ C`:

```text
∫ x^2 dx  → x^3/3 + C
```

The multiple-integral forms let you set every differential variable, integration order, and bound. Their bounds must be finite; an inner bound may use variables from outer integrations only. A scalar line integral uses `f(x,y)`, `x(t)`, and `y(t)`; a vector line integral uses `P dx + Q dy`. Surface forms use a parameterized patch `r(u,v)` and finite parameter bounds. For flux, choose the normal orientation `r_outer × r_inner` or its reverse.

### Derivative

Choose **Indefinite Derivative** from the `∫` chooser for a symbolic derivative. `SHIFT + ∫` remains a shortcut to the derivative template: leave its point field blank for a symbolic derivative, or enter a point for a numerical derivative.
...
Press `∫` in Complex mode for the LCD-only complex-calculus chooser; it does not use `OPTN` or a dialog. Choose complex definite or indefinite integration, a complex double integral, or a contour integral, then set the differential variables, order, and bounds in the displayed fields. Complex double-integral bounds follow the same finite nested-bound rule as real multi-integrals. A contour integral asks for `f(z)`, a parameterized path `z(t)`, the complex variable, a real parameter, and finite parameter bounds. For example, `f(z)=1/z`, `z(t)=exp(i*t)`, `0` to `2*pi` returns `2πi`. Paths that cross a pole are rejected.
...
- **Equation / Function:** solve simultaneous linear systems with 2–4 unknowns, polynomial roots of degree 2–4, or a symbolic first- or second-order ordinary differential equation. In the LCD form, choose the dependent and independent variables; enter `dy/dx=y` or `d2y/dx2+y=0`; and optionally add initial conditions. A first-order initial-value problem uses `y(x0)`; a second-order one also uses `y'(x0)`. For `x²-5x+6`, enter polynomial coefficients `1,-5,6`.
...
Click **Save** to store settings under the current Windows user profile in the app's SQLite database: normally `%LOCALAPPDATA%\ScientificCalculator\settings.db`, or the fixed `%USERPROFILE%\.scientific_calculator\ScientificCalculator\settings.db` fallback when `LOCALAPPDATA` is unavailable. The same database keeps the last 10 calculation expressions and displayed results. **Clear History** asks for confirmation, then removes only those saved calculations. Closing through the window close button or `SHIFT + AC` also saves the active settings. **Reset to Defaults** clears both saved settings and calculation history, then restores the default configuration; it is separate from `ON` restart. The uninstaller removes either fixed app-data location so a reinstall starts clean.
