...
#define MyAppExeName "ScientificCalculator.exe"
  ...
  #define MyOutputBaseFilename "ScientificCalculator_Setup"
#endif
...
begin
  { USERPROFILE is not an Inno Setup directory constant. Match Python's
    Windows home-directory resolution through the process environment. }
  ...
  Result := ProfileDirectory + '\.scientific_calculator\ScientificCalculator';
end;
...
begin
  ...
  begin
    { Ensure every installed file can be removed immediately. Ignore the
      harmless "process not found" exit code when the app is already closed. }
    Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM "{#MyAppExeName}"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    FallbackDirectory := FallbackDataDirectory;
    if FallbackDirectory <> '' then
      { Remove only the exact app-owned fallback directory, including SQLite
        sidecars and logs; never delete the parent user-profile folder. }
      DelTree(FallbackDirectory, True, True, True);
  end;
end;
